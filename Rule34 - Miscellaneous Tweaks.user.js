...
function loadTweaks()
{
	...
	content = !recursion && navbar && document.getElementById("content");
	if( !content )
		return;
	...
	settingsLink.addEventListener( "mousedown", function()
	{
		...
		else
		{
			...
			settingsDiv.appendChildX({ tag:"hr" }, "This is just something I wanted to try. The original code was done by Mango_2 @ ", { tag:"a", href:"https://greasyfork.org/en/scripts/3467-danbooru-miscellaneous-tweaks", text:"Danbooru - Miscellaneous Tweaks" }, "!" );
...
function initialize()
{
	...
	if( typeof(GM_getValue) == "undefined" || GM_getValue('a', 'b') == undefined )
	{
		...
		GM_deleteValue = function(a){ localStorage.removeItem("rule34_miscellaneous."+a); }
		GM_openInTab = function(a){ return window.open(a,a); }
		GM_getValue = function(name, defaultValue)
		{
			var value = localStorage.getItem("rule34_miscellaneous."+name);
			if( !value )
				return defaultValue;
		...
		GM_setValue = function(name, value)
		{
			...
			localStorage.setItem("rule34_miscellaneous."+name, value);
		}
		GM_listValues = function()
		{
			let j = 0, list = new Array(localStorage.length);
			for( let i = 0; i < localStorage.length; i++ )
				if( /^rule34_miscellaneous/.test( localStorage.key(i) ) )
					list[j++] = localStorage.key(i).replace(/^rule34_miscellaneous./,'');
			return list;
		}
		GM_xmlhttpRequest = function(obj)
		{
			//Unlike the Greasemonkey function, XMLHttpRequest can't access sites outside Danbooru
			if( !/^https?:..([^.]+\.)?rule34.xxx\//.test(obj.url) )
				return;
    ...
    let navMenu = document.querySelectorAll("#subnavbar");
    if( navMenu.length > 0 )
    {
        navbar = navMenu[0];
        subnavbar = navMenu[1];
        content = !recursion && document.getElementById("content");
		if( !subnavbar )
		{
...
function changePlusMinusLinks(addListeners)
{
	...
	setTimeout( function() //0-timeout to let the custom tags get added
	{
		...
		for( let i = 0; i < tagList.snapshotLength; i++ )
		{
			let links = tagList.snapshotItem(i).getElementsByTagName("a");
			if( links.length < 4 )
			{
...
function scoreThumbnails( appendScore, appendFavcount )
{
	...
	function giveSomething(e)
	{
        ...
        let articles;
        if( !e )
            articles = document.querySelectorAll("span.thumb");
        else if( e.target.classList.contains("post-preview") )
            articles = [ e.target ];
        else
            articles = e.target.querySelectorAll("span.thumb");
            ...
            if( appendScore && !outerBlock.getElementsByClassName("post-votes")[0] )
            {
                ...
                outerBlock.appendChild( createElementX({ tag: "span", class:"post-votes inline-flex gap-1", "data-id":postID }) ).innerHTML =
                    '<a class="post-upvote-link inactive-link" data-remote="true" rel="nofollow" data-method="post" href="/posts/'+postID+'/votes?score=1">' +
                        '<svg class="icon svg-icon upvote-icon" viewBox="0 0 448 512"><use fill="currentColor" href="/packs/static/images/icons-00aefbd7f752f10ede25.svg#arrow-alt-up"></use></svg></a>' +
                    ...
                    '<a class="post-downvote-link inactive-link" data-remote="true" rel="nofollow" data-method="post" href="/posts/'+postID+'/votes?score=-1">' +
                        '<svg class="icon svg-icon downvote-icon" viewBox="0 0 448 512"><use fill="currentColor" href="/packs/static/images/icons-00aefbd7f752f10ede25.svg#arrow-alt-down"></use></svg></a>';
            }
...
function blacklistTags( blacklist )
{
    ...
    blacklist = new RegExp( "(^|.* )("+blacklist.join("|").replace(/(\$|\^|\)|\(|\+|\.|\[|\])/g,"\\$1").replace(/\*/g,"[^\s ]*")+")( .*|$)", "i" );
    ...
    MS_observeInserts( function(e)
    {
        ...
        if( !e )
        {
            found = Array.from( document.querySelectorAll("img.preview") );
        ...
        else if( e.target.tagName == "IMG" && e.target.classList && e.target.classList.contains("preview") )
            found = [ e.target ];
        else if( e.target.querySelectorAll )
            found = e.target.querySelectorAll("img.preview");
        else
            return;
        
        if( found.length != 0 && !found[0].hasAttribute("alt") || ("title") )
...
//			return;
		let tags = elem.getAttribute("alt") || elem.getAttribute("tags");
        if( !tags )
            return;
		...
		var image = badTags.length && !elem.getAttribute("blacklisted") && elem; //document.getElementsByTagName("img")[1];
		...
		let blackLink = createElementX({ tag: "a", href:image.parentNode.href, text:('hidden ('+badTags+')').replace(/,/g,', '), title:( image.title || "" ) });
        ...
        for( let parent = image; parent != elem; parent = parent.parentNode )
            if( parent.href )
            {
                ...
                elem.parent.style.textAlign = "center";
                break;
            }
		...
		elem.parentNode.insertBefore( blackLink, image );
		...
		elem.addEventListener( "click", function(e)
		{
			...
			//Swap styles
			let oldImageStyle = image.style.display = "block";
			image.style.display = blackLink.style.display = "none";
...
function addTagSubscription( title, useTimestamp, refreshInterval, maxThumbs, maxExpanded, isIntersect, queryList )
{
	...
	function runQuery( argIndex, page )
	{
		...
		//Fetch next page
		MS_requestAPI( "/index.php?page=dapi&s=post&q=index&json=1&id="+encodeURIComponent(queryList[argIndex].search)+"&pid="+page, function(responseDetails)
		{
			var result;
...
function postQueueTweaks( badTags, disapproveTags, swapKey, swapClick, autoVote, limit, moveButtons )
{
    ...
    function swapImage(post)
    {
        ...
        let media = post.getElementsByTagName("img");
        ...
        if( !media[1].firstChild.src )
        {
            ...
            let art = post.getElementsByTagName("img");
...
function MS_requestAPI( link, loadFun, errFun )
{
	...
	{
		method: "GET",
		url:location.protocol+"//"+location.host+link+( link.indexOf("limit=") < 0 ? ( link.indexOf("?") > 0 ? "&limit=42" : "?limit=42" ) : "" ),
		onload:loadFun,
...
function buildThumbs(json, limit)
{
	...
	for( i = 0; limit-- > 0 && i < json.length; i++ )
	{
        ...
        let article = createElementX({ "tag":"span",
				"class":"preview",
				"id":"s"+json[i].id,
//				"data-id":json[i].id,
...
//                article.setAttribute( "data-"+size.type, size.url );
		...
		article.appendChildX({ tag:"a", href:"index.php?page=post&s=view&id="+json[i].id, style:"" })
            .appendChildX({ src:json[i].preview_url, alt:json[i].tags, title:json[i].tags+" score:"+json[i].score+" rating:"+json[i].rating});
...
function MS_usingArticles(artNodes,artConsumer)
{
	...
	//Build query string
	let query = "/index.php?page=dapi&s=post&q=index&json=1"; //+totalAttr.toString()+"&tags=status:any+id:";
	for( let art of artNodes )
    {
        if( !art.hasAttribute("id") )
            art.setAttribute("id", art.id.substring(5) );
		query += art.getAttribute("id")+",";
    }
